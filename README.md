# GoogleSERP - is the async fork of [`googleserp` (Yet another googlesearch)](https://github.com/opsdisk/yagooglesearch)
...
`googleserp` is a Python library for executing intelligent, realistic-looking, and tunable Google searches.  It
...
```python
import asyncio
from googleserp import SearchClient
...
`googleserp` will find anything close to that.  Testing shows that at most, about 400 results are returned.  If you
...
Low and slow is the strategy when executing Google searches using `googleserp`.  If you start getting HTTP 429
responses, Google has rightfully detected you as a bot and will block your IP for a set period of time. `googleserp`
...
If `googleserp` detects an HTTP 429 response from Google, it will sleep for `http_429_cool_off_time_in_minutes`
...
The goal is to have `googleserp` worry about HTTP 429 detection and recovery and not put the burden on the script
using it.

If you do not want `googleserp` to handle HTTP 429s and would rather handle it yourself, pass
...
`googleserp` supports the use of a proxy.  The provided proxy is used for the entire life cycle of the search to
...
To use a proxy, provide a proxy string when initializing a `SearchClient` object:
...
* `socks5` - "causes the DNS resolution to happen on the client, rather than on the proxy server."  You likely **do
  not** want this since all DNS lookups would source from where `googleserp` is being run instead of the proxy.
...
1) Instantiating the `SearchClient` object:
...
If you want to use multiple proxies, that burden is on the script utilizing the `googleserp` library to instantiate
a new `SearchClient` object with the different proxy. Below is an example of looping through a list of
proxies:
